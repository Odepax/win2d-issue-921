...
#nullable restore
		...
		if (result == HRESULT.E_INVALIDARG)
			throw new Exception("The value passed in is not valid.");

		else if (result == HRESULT.E_ACCESSDENIED)
			throw new Exception("The DPI awareness was already set, either by calling this API previously, or through the application manifest.");
		...
		fixed (char* lpszClassName = WindowClassName)
        {
            var wndClassEx = new WNDCLASSEXW
            {
                ...
                lpfnWndProc = Win32WindProcAntiGcReference = WndProc,

				style = CS_HREDRAW | CS_VREDRAW | CS_DBLCLKS | CS_OWNDC,

				hIcon = LoadIcon((HMODULE) hInstance.DangerousGetHandle(), IDI_APPLICATION),
    ...
    LRESULT WndProc(HWND hWnd, uint message, WPARAM wParam, LPARAM lParam)
    {
        ...
        {
            case WM_PAINT:
                BeginPaint(hWnd, out PAINTSTRUCT lpPaint);
				// TODO: should we use RunRenderPass() here?
				EndPaint(hWnd, in lpPaint);
				break;
